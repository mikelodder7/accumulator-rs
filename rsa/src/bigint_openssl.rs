...
impl std::fmt::Display for BigInt {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "BigInt {{ value: {} }}", self.value.to_dec_str().unwrap())
    }
}
...
impl BigInt {
    ...
    pub fn bezouts_coefficients(&self, rhs: &Self) -> (Self, Self) {
        ...
        if self.value == zero && rhs.value == zero {
            return (BigInt { value: zero }, BigInt { value: BigNum::new().unwrap() })
        }
        if self.value == zero {
            return (BigInt{ value: zero }, BigInt { value: BigNum::from_u32(1).unwrap() })
        }
        if rhs.value == zero {
            return (BigInt{ value: BigNum::from_u32(1).unwrap() }, BigInt { value: zero })
        }
        ...
        return (BigInt { value: old_s }, BigInt { value: old_t })
    }
    ...
    pub fn lehmer_gcd(&self, rhs: &Self) -> (Self, Self) {
        ...
        (BigInt{ value: ua }, BigInt{ value: y })
    }
}
...
macro_rules! ops_impl {
    ($t:ident, $ts:ident, $f:ident, $fs:ident,$i:ident, $is:ident) => {
        ...
        impl<'a, 'b> $t<&'b BigInt> for &'a BigInt {
            type Output = BigInt;
        ...
        impl $ts for BigInt {
            fn $fs(&mut self, rhs: BigInt) {
                self.$is(&rhs)
            }
        }

        impl $ts<&BigInt> for BigInt {
            fn $fs(&mut self, rhs: &BigInt) {
                self.$is(rhs)
            }
        }
...
impl From<&[u8]> for BigInt {
    fn from(value: &[u8]) -> Self {
        Self { value: BigNum::from_slice(value).unwrap() }
    }
}

impl Into<Vec<u8>> for BigInt {
    fn into(self) -> Vec<u8> {
        self.value.to_vec()
    }
}
...
mod tests {
    ...
    fn test_bezouts_coefficients() {
        ...
        let a = BigInt::from(8890919903463260501u64);
        let b = BigInt::from(4108249713441620807u64);
        let (x, y) = a.bezouts_coefficients(&b);
        assert_eq!(&a * &x + &b * &y, BigInt::from(1));
        // let (x1, y1) = a.lehmer_gcd(&b);
        // assert_eq!(x, x1);
        // assert_eq!(y, y1);

        let a = BigInt::from("59066688664129022771864664899854388241934199");
        let b = BigInt::from("37835724723609910915097429455428534256391567");
        let (x, y) = a.bezouts_coefficients(&b);
        assert_eq!(a * x + b * y, BigInt::from(1));
    }
}