use crate::{Poke2Proof, accumulator::Accumulator, nonwitness::NonMembershipWitness, FACTOR_SIZE, b2fa};
use common::{
    Field,
    bigint::BigInteger,
    error::*,
...
impl NonMembershipProof {
    ...
    pub fn verify<B: AsRef<[u8]>>(&self, accumulator: &Accumulator, nonce: B) -> bool {
        ...
        let v_res = self.proof_v.verify_with(&self.v, &accumulator, nonce);
        g_res && v_res
...
mod tests {
    ...
    fn proof_test() {
        ...
        let proof = NonMembershipProof::new(&witness, &acc, nonce);
        assert!(!proof.verify(&acc, nonce));
        assert_eq!(proof.to_bytes().len(), 2 * Poke2Proof::SIZE_BYTES + 2 * FACTOR_SIZE);
    }
}